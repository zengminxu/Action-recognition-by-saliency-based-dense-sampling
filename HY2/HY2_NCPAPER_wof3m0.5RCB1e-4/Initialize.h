...
// xzm
void InitMVFInfo(MVFInfo* mvfInfo, int mvf_length, int init_gap)
{
	mvfInfo->length = mvf_length;
	mvfInfo->gap = init_gap;
}
...
void InitSeqInfo(SeqInfo* seqInfo, std::string video)//char* video)
{
...
void usage()
{
	...
	// pxj
	fprintf(stderr, "  -u [show track]			 Set show_track = 1, if you want to visualize the trajectories (default: 0)\n");
	fprintf(stderr, "  -o [output filename]		 The output file for Improved Dense trajectory (default: None)\n");
	// xzm
	fprintf(stderr, "  -p [specific idt sample num]	 The iDT-RCB will begin if sampled num achieves the specific idt number (default: 0)\n");
}
...
bool arg_parse(int argc, char** argv)
{
	...
	while((c = getopt (argc, argv, "hS:E:L:W:N:s:t:A:I:H:o:u:p:")) != -1)
	switch(c) {
		case 'S':
