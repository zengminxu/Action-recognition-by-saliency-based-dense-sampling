...
void CmSaliencyGC::MergeGMMs()
{
	...
	for (int y = 0; y < _img3f.rows; y++) {
		for (int x = 0; x < _img3f.cols; x++){
			...
			// xzm 20150316 fixed "Expression: vector subscript out of range"!!
			float cost = 0;
			int i1 = 0, i2 = 0;				 
			i1 = pIdx[0].second; 
			if(_NUM < 2){	 
				i2 = pIdx[0].second;
				cost = min(pIdx[0].first, pIdx[0].first);
			}
			else{
				i2 = pIdx[1].second;
				cost = min(pIdx[0].first, pIdx[1].first);
			}
...
*/
int CmSaliencyGC::XZM(Mat img, Mat& salMask)
{
	vecS namesNE, des;
	img.convertTo(img, CV_32FC3, 1.0/255);
	CmSaliencyGC sGC(img, "outGCName.jpg");
	sGC.HistgramGMMs();
	Mat sal = sGC.GetSaliencyCues();

	// xzm for cmm 20150316 fixed "Expression: vector subscript out of range"!!
	if(sGC._NUM == 1)
		salMask = Mat(img.size(),CV_8UC1,Scalar(255));
	else
		salMask = sal*255;
