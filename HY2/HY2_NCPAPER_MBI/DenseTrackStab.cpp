...
#endif
{
	...
	std::vector<std::list<Track> > xyScaleTracks_rc, xyScaleTracks_mbi;	
	...
	while(true) {		
		...
		if(frame_num == start_frame) {	// 先对第一帧进行初始化操作
			...
			for(int iScale = 0; iScale < scale_num; iScale++) {// 将每个视频帧在不同尺度下的图像进行密集采样
				...
				xzmDenseSample(prev_grey_pyr[iScale], points, points_rc, points_rcb, points_mbi, 
								quality, min_distance, salMask_pyr[iScale]);
		...
		for(int iScale = 0; iScale < scale_num; iScale++) {
			...
			// pxj比wh多加了一个参数MBI
			MbhComp(flow_warp_pyr[iScale], MBI_pyr[iScale], mbhMatX->desc, mbhMatY->desc, mbhInfo);
			...
			xzmDenseSample(grey_pyr[iScale], points, points_rc, points_rcb, points_mbi, 
...
int predictHY2()
{
	...
	string resultPath="/dawnfs/users/xuzengmin/code/HY2_NCPAPER_MBI/";
...
int predictHMDB51()
{
	...
	string resultPath="/dawnfs/users/xuzengmin/code/HMDB51_NCPAPER_MBI/";
...
int predictUCF50()
{
	...
	string resultPath="/dawnfs/users/xuzengmin/code/UCF50_NCPAPER_MBI/";
