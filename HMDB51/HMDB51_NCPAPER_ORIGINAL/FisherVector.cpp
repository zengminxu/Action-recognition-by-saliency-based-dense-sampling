						   ...
						   int gmmNum, string resultPath, string descriptorType, string manifoldType)
{
	...
	// 限定训练码本所需的视频数不超过SAMPLE_VIDEO段视频
	int videoNum = actionSet.size();
	multimap<string, string> videoSampling;
	if( actionSet.size() > SAMPLE_VIDEO )
		videoNum = SAMPLE_VIDEO;

	RNG rng(getTickCount());	// 取当前系统时间作为随机数种子
	Mat randMat(videoNum, 1, CV_32S);
	for(int r=0; r<randMat.rows; r++)
		randMat.at<int>(r,0) = r;
	randShuffle(randMat, 1, &rng);
	...
	for(int r=0; r<randMat.rows; r++){
		...
		for(iter = actionSet.begin(); iter != actionSet.end(); iter++){
			if(count++ == randRow){
				videoSampling.insert(map<string, string>::value_type(iter->first, iter->second));
				break;
			}
		}
	}
...
int FisherVector::newSamplePCAWhiten(Mat src, Mat& dst, string resultPath, string descriptorType, string PCA_DATA_TYPE)
{
	...
	// PCA白化。如果 f 是以行向量存储特征的零均值样本矩阵，则白化公式为：  X = f*U*△    （1）
...
int FisherVector::pcaWhiten(Mat src, Mat& dst, string resultPath, int pcaNum, string descriptorType, string PCA_DATA_TYPE)
{
	...
	float whiteningRegul = 0.01;//白化正则项
    float eplison = 0.00001;    //eplison为防止分母为0
	
	Mat eigvalues = pca.eigenvalues;
	eigvalues = eigvalues + whiteningRegul * eigvalues.at<float>(0,0);	// eigvalues.at<float>(0,0)为最大的特征值
    ...
    for(int i=0; i<eigvalues.rows; i++)
        for(int j=0; j<eigvalues.cols; j++)
            sqrtEigMat.at<float>(i,j) = 1.0 / sqrt(eigvalues.at<float>(i,j) + eplison);
    diagMat = Mat::diag(sqrtEigMat);

	// 白化。如果样本矩阵f 以行向量存储特征样本，白化公式：  X = f*U*△	    （1） 或者	X = U'*f'*△     （3）
	// 如果样本矩阵f 是以列向量存储特征样本，则白化公式为：  X = △*U'*f    （2）
	dst = projMat * diagMat;
	//dst = projMat; // 不白化

	// 验证projMat == U'*f' 或者 projMat == f*U，其中f 为行向量样本X 去均值化后的矩阵
	//for(int i=0; i<src.rows; i++)
	//	for(int j=0; j<src.cols; j++)
	//		src.at<float>(i,j) = src.at<float>(i,j) - pca.mean.at<float>(0,j);
	// 此时，projMat == pca.eigenvectors*src.t() 或者	projMat == src*pca.eigenvectors.t()

	FileStorage fspca;
	string pca_file = resultPath + "pca" + descriptorType + ".xml";
	//编码用到3个参数：means, 对角矩阵diag, 投影矩阵U==pca.eigenvectors.t()
