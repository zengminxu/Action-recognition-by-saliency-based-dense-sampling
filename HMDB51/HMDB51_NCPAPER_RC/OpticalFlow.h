...
FarnebackPolyExp( const Mat& src, Mat& dst, int n, double sigma )
{
    ...
    for( y = 0; y < height; y++ )
    {
        ...
        for( k = 1; k <= n; k++ )
        {
            ...
            srow0 = (float*)(src.data + src.step*std::max<int>(y-k,0));
            srow1 = (float*)(src.data + src.step*std::min<int>(y+k,height-1));
                                  ...
                                  bool update_matrices )
{
    ...
    int m = block_size/2;
    int y0 = 0, y1;
    int min_update_stripe = std::max<int>((1 << 10)/width, block_size);
    ...
    for( y = 0; y < height; y++ )
    {
        ...
        for( i = 0; i <= m; i++ )
        {
            srow[m-i] = (const float*)(matM.data + matM.step*std::max<int>(y-i,0));
            srow[m+i] = (const float*)(matM.data + matM.step*std::min<int>(y+i,height-1));
        }
						 ...
						 std::vector<float>& fscales, int poly_n, double poly_sigma)
{
    ...
    for(int k = 0; k < poly_exp_pyr.size(); k++)
    {
        ...
        smooth_sz = std::max<int>(smooth_sz, 3);
