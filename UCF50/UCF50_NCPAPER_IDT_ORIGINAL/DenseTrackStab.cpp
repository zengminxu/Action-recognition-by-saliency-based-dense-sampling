...
int makeHY2splits(vector<string>&, string, string);
...
#endif
{
	...
	std::cout << "extract features " << file_name << "...." << endl;

	VideoCapture capture;
	std::string video = lp->datasetPath + lp->filename + lp->extname;
	capture.open(video);
	...
	while(true) {		
		...
		if(frame_num == start_frame) {	// 先对第一帧进行初始化操作
			...
			for(int iScale = 0; iScale < scale_num; iScale++) {// 将每个视频帧在不同尺度下的图像进行密集采样
				...
				DenseSample(prev_grey_pyr[iScale], points, quality, min_distance);	// 根据邻域大小W=5进行特征点密集采样. min_distance就是论文的W.
		...
		for(int iScale = 0; iScale < scale_num; iScale++) {
			...
			//在每个尺度iScale下跟踪特征点。测试视频person01.avi被分为4个尺度，tracks每次对应一种尺度下所有特征点所形成的所有轨迹(链表形式存储)
			std::list<Track>& tracks = xyScaleTracks[iScale];		
			for (std::list<Track>::iterator iTrack = tracks.begin(); iTrack != tracks.end();) {
				...
				// 将特征点point当前跟踪的帧数index+1，再加该点加入iTrack->point[index]
				...
				if(iTrack->index >= trackInfo.length) {
					...
					float mean_x(0), mean_y(0), var_x(0), var_y(0), length(0);										// 初始化为0. 注意：此length为轨迹长度，初始化为0后作参数输入IsValid函数
					if(IsValid(trajectory, mean_x, mean_y, var_x, var_y, length) && IsCameraMotion(displacement)) {	// 检查是否动作轨迹。如果轨迹中单次位移小于1就认为是动作轨迹，否则为相机运动产生的轨迹
...
int predictHY2()
{
	...
	string resultPath="/dawnfs/users/xuzengmin/code/Hollywood2/";
	...
	makeHY2splits(actionType, datasetPath, resultPath);
	string splitFile = resultPath + datasetName + "_splits.txt";
...
int predictHMDB51()
{
	...
	string resultPath="/dawnfs/users/xuzengmin/code/HMDB51_IDT_TEST/";
...
int predictJHMDB()
{
	...
	string resultPath="/dawnfs/users/xuzengmin/code/JHMDB_split1/";
#endif
...
int predictUCF50()
{
	...
	string resultPath="/dawnfs/users/xuzengmin/code/UCF50_NCPAPER_IDT_ORIGINAL/";
...
int makeHY2splits(vector<string> &actionType, string datasetPath, string resultPath){
	string action[12] = {"AnswerPhone","DriveCar","Eat","FightPerson","GetOutCar","HandShake","HugPerson","Kiss","Run","SitDown","SitUp","StandUp"};
	string split_file = resultPath + "hollywood2_test_train_splits.txt";
	fstream split;
	split.open(split_file.c_str(), ios::out);
	if(!split.is_open()){
		cout << "Error: Could not open file '" << split_file << "'." << endl;
